...
- [Chapter 1 引论](#chapter-1-引论)
    - [1.1 什么是大数据及其特点](#11-什么是大数据及其特点)
        - [1.1.1 特点](#111-特点)
    - [1.2 知识发现的流程以及核心](#12-知识发现的流程以及核心)
        - [1.2.1 数据中的知识发现（KDD）](#121-数据中的知识发现kdd)
            - [**核心 ： 数据挖掘**](#核心--数据挖掘)
    - [1.3 什么是数据挖掘，有哪些任务](#13-什么是数据挖掘有哪些任务)
        - [1.3.1 主要任务](#131-主要任务)
    - [1.4 主流会议与期刊](#14-主流会议与期刊)
        - [1.4.1 会议](#141-会议)
        - [1.4.2 期刊](#142-期刊)
- [Chapter 2 认识数据与预处理](#chapter-2-认识数据与预处理)
    - [2.1 认识数据](#21-认识数据)
        - [2.1.1.基本概念](#211基本概念)
        - [2.1.2 属性类型](#212-属性类型)
            - [分类型（Categorical）](#分类型categorical)
            - [数值型（Numerical）](#数值型numerical)
            - [二元属性](#二元属性)
        - [2.1.3 数据类型](#213-数据类型)
        - [2.1.4 数据的描述](#214-数据的描述)
            - [中心性描述](#中心性描述)
            - [散度描述](#散度描述)
        - [2.1.5 数据的相似性度量](#215-数据的相似性度量)
            - [标称属性数据](#标称属性数据)
            - [二元属性数据](#二元属性数据)
                - [例题](#例题)
            - [数值型数据的相异性](#数值型数据的相异性)
                - [闵科夫斯基距离](#闵科夫斯基距离)
            - [序数属性的邻近性度量](#序数属性的邻近性度量)
            - [混合类型属性的相异性](#混合类型属性的相异性)
            - [余弦相似性](#余弦相似性)
    - [2.2 数据预处理](#22-数据预处理)
        - [2.2.1 主要任务](#221-主要任务)
            - [数据清理](#数据清理)
            ...
            - [数据集成](#数据集成)
                - [冗余数据处理](#冗余数据处理)
                    - [例题](#例题-1)
            - [数据归约](#数据归约)
- [Chapter 3 关联规则挖掘](#chapter-3-关联规则挖掘)
    - [3.1 什么是关联规则挖掘](#31-什么是关联规则挖掘)
        - [3.1.1 一般来说该过程包含两步](#311-一般来说该过程包含两步)
        - [3.1.2 基本概念](#312-基本概念)
    - [3.2 Apriori算法](#32-apriori算法)
        - [3.2.1 相关定义](#321-相关定义)
        - [3.2.2 步骤](#322-步骤)
            - [由频繁项集产生关联规则](#由频繁项集产生关联规则)
            - [python代码如下：](#python代码如下)
        - [3.2.3 提高Apriori算法的方法](#323-提高apriori算法的方法)
            - [Hash-based itemset counting](#hash-based-itemset-counting)
            - [Transaction reduction](#transaction-reduction)
            - [Partitioning](#partitioning)
            - [抽样](#抽样)
    - [3.3 FP-growth](#33-fp-growth)
        - [3.3.1 构建FP树](#331-构建fp树)
            - [举例说明](#举例说明)
            - [FP Tree 挖掘](#fp-tree-挖掘)
        - [3.3.2 FP Tree算法归纳](#332-fp-tree算法归纳)
        - [3.3.3 关联模式评估](#333-关联模式评估)
- [Chapter 4 分类与预测](#chapter-4-分类与预测)
    - [4.1 监督学习VS无监督学习](#41-监督学习vs无监督学习)
    - [4.2 模型分类](#42-模型分类)
    - [4.3 决策树](#43-决策树)
        - [4.3.1 决策树构造](#431-决策树构造)
            - [具体流程](#具体流程)
            - [属性选择度量](#属性选择度量)
                - [信息增益（ID3）](#信息增益id3)
                - [信息增益率](#信息增益率)
                - [Gini指标](#gini指标)
        - [4.3.2 决策树过拟合问题](#432-决策树过拟合问题)
        - [4.3.3 由决策树提取分类规则](#433-由决策树提取分类规则)
    - [4.4 KNN（K近邻算法——K-Nearest Neighbor）](#44-knnk近邻算法k-nearest-neighbor)
        - [4.4.1 KNN的基本思想](#441-knn的基本思想)
        - [4.4.2 算法计算步骤](#442-算法计算步骤)
        - [4.4.3 算法的优缺点](#443-算法的优缺点)
        - [4.4.4 KNN常见的问题](#444-knn常见的问题)
        - [4.4.5 实现代码](#445-实现代码)
    - [4.5 朴素贝叶斯（Naive Bayes）](#45-朴素贝叶斯naive-bayes)
    - [4.6 支持向量机 （Suppport Vector Machine）SVM](#46-支持向量机-suppport-vector-machinesvm)
            - [结构风险最小化](#结构风险最小化)
            - [SVM小结](#svm小结)
    - [4.7 人工神经网络（Artificial Neural Network）ANN + 感知机 + BP](#47-人工神经网络artificial-neural-networkann--感知机--bp)
        - [4.7.1 ANN](#471-ann)
        - [4.7.2 感知机模型](#472-感知机模型)
            - [模型](#模型)
            - [学习过程](#学习过程)
            - [感知机简单实现](#感知机简单实现)
        - [4.7.3 BP](#473-bp)
            - [python实现简单BP](#python实现简单bp)
    - [4.8 集成学习](#48-集成学习)
        - [4.8.1 Bagging(随机森林就是其一个应用)](#481-bagging随机森林就是其一个应用)
        - [4.8.2 Boosting](#482-boosting)
        - [4.8.3 Stacking](#483-stacking)
    - [4.9 分类器的评价标准](#49-分类器的评价标准)
                - [类分布不平衡的问题](#类分布不平衡的问题)
- [Chapter 5 聚类分析（Cluster Analysis）](#chapter-5-聚类分析cluster-analysis)
    - [5.1 认识聚类](#51-认识聚类)
        - [5.1.1 聚类算法分类](#511-聚类算法分类)
            - [划分方法](#划分方法)
                - [K-means](#k-means)
                - [K—中心点（K-medoids）](#k中心点k-medoids)
            - [层次聚类方法](#层次聚类方法)
            - [基于密度的聚类方法](#基于密度的聚类方法)
                - [DBSCAN](#dbscan)
            - [网格聚类](#网格聚类)
        ...
        - [离群点（孤立点）检测](#离群点孤立点检测)
            ...
            - [基于偏离的孤立点检测](#基于偏离的孤立点检测)
            - [基于密度的方法](#基于密度的方法)
- [Chapter 6 大数据分析](#chapter-6-大数据分析)
    - [6.1 哈希技术](#61-哈希技术)
        - [数据流挖掘](#数据流挖掘)
...
$$ d(i,j)=\frac{p-m}{p} $$
<a href="https://www.codecogs.com/eqnedit.php?latex=d(i,j)=\frac{p-m}{p}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?d(i,j)=\frac{p-m}{p}" title="d(i,j)=\frac{p-m}{p}" /></a>
